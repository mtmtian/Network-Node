...
首次部署后可把主机专用私钥放在 `profiles/dmit/ssh/id_rsa.pem`。后续在项目根目录直接运行
`./deploy-vps.sh` 即可自动复用已保存的 IP 和私钥，不需要依赖仓库外路径。
...
config/                         不含密钥的配置模板
docs/                           架构说明与排障文档
profiles/gcloud/                GCloud 独立状态与 clients/（不提交）
profiles/dmit/                  DMIT 独立状态、clients/ 与 ssh/（不提交）
```
...
- `profiles/dmit/clients/dmit-mac.yaml`
- `profiles/dmit/clients/dmit-iphone.yaml`

生成器只替换当前 profile 前缀的文件，例如 DMIT 只处理 `dmit-*.yaml`，不会覆盖
`gcloud-*.yaml`。客户端 YAML 默认权限为 `600`，因为其中含节点地址、UUID 和密码；
