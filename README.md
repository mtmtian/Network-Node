...
首次运行任一入口都会从 `config/deploy.conf.example` 创建 `deploy.conf`。
...
| `DEVICES` | `mac iphone ipad laptop spare` | 每个设备生成独立凭据和 YAML |
...
部署成功后，每台设备会在 `clash-configs/` 下得到一份 YAML。
...
- 只重新生成客户端 YAML：`python3 core/gen-clash.py`
